...
/// `FragmentedKeyManager` `FKR` is the central class in the FragmentedKeys library.
...
///
///
final class FragmentedKeyManagerDeprecated {
    static let shared: FragmentedKeyManagerDeprecated = {
        guard let manager = _shared else {
            fatalError("FragmentedKeyManager has not been initialized correctly.")
        }
        return manager
    }()
    private static var _shared: FragmentedKeyManagerDeprecated?

    /// The environment that provides settings and storage managers.
    let environment: EnvironmentDeprecated
    ...
    init(environment: EnvironmentDeprecated) {
        guard FragmentedKeyManagerDeprecated._shared == nil else {
            fatalError("FragmentedKeyManager has already been initialized.")
        }
        self.environment = environment
        FragmentedKeyManagerDeprecated._shared = self
    }
